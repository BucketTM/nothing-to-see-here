 ...
 */
syscall mapAddressRange(pgtbl pagetable, ulong vaddr, ulong paddr, ulong length, int attr)
{
    ulong end;
...
syscall mapPage(pgtbl pagetable, ulong vaddr, ulong paddr, int attr)
{
     ...
     *
     * Call sfence_vma once finished to flush TLB
    ...
    // DEBUGGING LINE:
    // kprintf("mapPage(pt:0x%lX, v:0x%lX, p:0x%lX, a:0x%03X)\r\n",
    //         (ulong)pagetable, vaddr, paddr, attr);

    vaddr = truncpage(vaddr);
    paddr = truncpage(paddr);

    for (level = 2; level > 0; level--)
    {
        int index = PX(level, vaddr);
        pte *entry = &pt[index];

        if (*entry & PTE_V)
        {
            pt = (pgtbl)PTE2PA(*entry);
        }
        else
        {
            pgtbl newpt = (pgtbl)pgalloc();
            if ((void *)newpt == (void *)SYSERR || newpt == NULL)
            {
                return SYSERR;
            }

            memset(newpt, 0, PAGE_SIZE);
            *entry = PA2PTE(newpt) | PTE_V;
            pt = newpt;
        }
    }

    pt[PX(0, vaddr)] = PA2PTE(paddr) | attr | PTE_V;
    sfence_vma();
