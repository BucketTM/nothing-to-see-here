...
#define RISCV_MSTATUS_MEI_BIT                                                  \
  (1 << 3) /* IRQs globally disabled on all privilege levels when set to 1. */
#define RISCV_MPP_TO_S_MODE                                                    \
  (1 << 11) /* Set the previous mode to S-mode so XINU can switch to S-Mode    \
               when completed. */
#define RISCV_SPP_TO_U_MODE                                                    \
  (1 << 8) /* Set the previous mode to U-mode so XINU can switch to U-Mode     \
              when completed. */
...
// Xinu virtual memory design choices
...
#define CONTEXT 34 /**< context record size in words         */
